...
function select_direct_bet_amount(element) {
    ...
    var adding_bet_amount = parseFloat($(element).find(".amt").text()).toFixed(
        2
    );
        ...
        success: function (result) {
            if (result.isSuccess) {
                ...
                if (section_no == 0) {
                    ...
                    let is_main_auto_bet_checked =
                        $("#main_auto_bet").prop("checked");
                ...
                if (section_no == 1) {
                    ...
                    let is_extra_auto_bet_checked =
                        $("#extra_auto_bet").prop("checked");
    ...
    if (bet_array[0] && bet_array[0].is_bet != undefined) {
        if (bet_array[0].section_no == 0) {
            if (is_main_auto_bet_checked) {
                ...
                $("#main_bet_section").find("#cancle_button #waiting").show();
                $("#main_bet_section").find("#cashout_button").hide();
                $("#main_bet_section .controls").removeClass(
                    "bet-border-yellow"
                );
                $("#main_bet_section .controls").addClass("bet-border-red");
            } else {
            ...
            if (is_extra_auto_bet_checked) {
                ...
                $("#extra_bet_section").find("#cancle_button #waiting").show();
                $("#extra_bet_section").find("#cashout_button").hide();
                $("#extra_bet_section .controls").removeClass(
                    "bet-border-yellow"
                );
                $("#extra_bet_section .controls").addClass("bet-border-red");
            } else {
...
function prevoius_game_bets(game_id) {
    $.ajax({
        url: website_url+"/previous_game_bet_list",
        data: {
            game_id: game_id,
        },
        type: "POST",
...
function place_bet_now() {
  ...
  //          console.log(JSON.stringify(bet_array,  null,2));
//alert(JSON.stringify(result.data, null,2));
                if (bet_array.length == 2) {
                    if (bet_array[0].section_no == 0) {
                        $("#main_bet_id").val(
                            result.data.return_bets[0].bet_id
                        );
                        $("#extra_bet_id").val(
                            result.data.return_bets[1].bet_id
                        );
                        bet_array[0].is_bet = 1;
                    }
                ...
                $("#main_bet_section .controls .navigation").removeClass(
                    "stop-action"
                );
            ...
            if (length > 0) {
                for (let i = 0; i < length; i++) {
                    if (parseFloat(result.data[i].multiplication) > 0) {
                        var multiplier = `<div class="${get_multiplier_badge_class(
                            result.data[i].multiplication
                        )} custom-badge mx-auto"> ${
                            result.data[i].multiplication
                        }x </div>`;
                    } else {
                    ...
                    $("#member_bet .mCSB_container").append(`
                        <div class="list-items bet_record_count ${
                            result.data[i].cash_out_amount > 0 ? "active" : ""
                        }">
                            <div class="column-1 users fw-normal">
                                ${result.data[i].date}
                            </div>
                            ...
                            <div class="column-4 fw-normal">
                                ${
                                    result.data[i].cash_out_amount > 0
                                        ? result.data[i].cash_out_amount +
                                          currency_symbol
                                        : "-"
                                }
                            </div>
...
// WINDOW REFRESH IN EVERY 10 SEC.
...
// WAITING FOR NEXT ROUND GAME LINE TIMER
$(".fill-line").bind(
