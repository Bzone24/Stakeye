...
use App\Models\GameBookmakerOdd;
...
class SiteController extends Controller {
    public function newIndex($categorySlug = null, $leagueSlug = null) {
        ...
        $categories = Category::withCount(['tournaments' => function ($query) {
            $query->whereHas('matches', function ($matchQuery) {
                $matchQuery->where('status', Status::ENABLE);
            });
        }])->get();
        ...
        if ($categorySlug) {
            ...
            $activeCategory = $categories->sortByDesc('tournaments_count')->first();
        }
        ...
        if ($leagueSlug) {
            $activeTournament = GameTournament::where('slug', $leagueSlug)
            ...
            $activeCategory = $activeTournament->sport;
        }

        if ($activeCategory && $activeCategory->tournaments->count()) {
            $tournaments = $this->filterByTournaments($activeCategory, $gameType);
            if (!$leagueSlug) {
                $activeTournament = $tournaments->first();
            }
        }
        $matches = GameMatch::where('competitionId', @$activeTournament->competition_id)
        ->with('markets')
        ->orderBy('id', 'desc')
        ->get();
    
        return view('Template::home', compact('pageTitle', 'categories', 'leagues', 'games', 'activeCategory', 'activeLeague','tournaments','matches'));
    }
    ...
    public function index($categorySlug = null, $leagueSlug = null) {
        ...
        $games = $games->where('league_id', @$activeLeague->id)->with(['teamOne', 'teamTwo'])->with(['questions' => function ($q) {
            $q->active()
                ->resultUndeclared()->select('id', 'game_id', 'title','market_type', 'locked')
    ...
    private function filterByTournaments($activeCategory, $gameType) {
        $tournaments = $activeCategory->tournaments();
        $gameType .= 'Game';
    
        return $tournaments->withCount("$gameType as game_count")
            ->orderBy('game_count', 'desc')
            ->where('status', Status::ENABLE) // Ensure only active tournaments
            ->get();
    }
