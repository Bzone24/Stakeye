                            ...
                            <tbody>
                                @forelse ($tournaments as $tournament)
                                    <tr>
                                        <td>
                                            <div class="user">
                                                <div class="thumb"><img src="{{ getImage(getFilePath('league') . '/' . $tournament->image, getFileSize('league')) }}" alt="@lang('image')" class="plugin_bg"></div>
                                                <span class="name">{{ __($tournament->name) }}</span>
                                            </div>
                                        </td>

                                        <td>{{ __($tournament->short_name) }}</td>
                                        <td>{{ $tournament->slug }}</td>
                                        <td>{{ __(@$tournament->sport->name) }}</td>
                                        <td>
                                            @php echo $tournament->statusBadge @endphp
                                        </td>
                                        <td>
                                            @php
                                                $tournament->image_with_path = getImage(getFilePath('league') . '/' . $tournament->image, getFileSize('league'));
                                            @endphp
                                            <div class="button--group">
                                                <button type="button" class="btn btn-sm btn-outline--primary cuModalBtn editBtn" data-category_id="{{ $tournament->category_id }}" data-image="{{ $tournament->image_with_path }}" data-resource="{{ $tournament }}" data-modal_title="@lang('Edit League')">
                                                    <i class="la la-pencil"></i>@lang('Edit')
                                                </button>

                                                @if ($tournament->status == Status::DISABLE)
                                                    <button class="btn btn-sm btn-outline--success ms-1 confirmationBtn" data-question="@lang('Are you sure to enable this tournament?')" data-action="{{ route('admin.league.flag', $tournament->id) }}">
                                                        <i class="la la-eye"></i> @lang('Enable')
                                                    </button>
                                                @else
                                                    <button class="btn btn-sm btn-outline--danger ms-1 confirmationBtn" data-question="@lang('Are you sure to disable this tournament?')" data-action="{{ route('admin.league.flag', $tournament->id) }}">
    ...
    <div id="cuModal" class="modal fade" tabindex="-1" role="dialog">
        <div class="modal-dialog modal-xl" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('admin.league.save') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-body">
                        <div class="row">
                            ...
                            <div class="col-lg-6">
                                ...
                                <div class="form-group">
                                    <label>@lang('Competition Region')</label>
                                    <input class="form-control" name="competitionRegion" type="number" value="{{ old('competitionRegion') }}" required />
                                </div>
                            </div>
                        </div>
                    </div>
