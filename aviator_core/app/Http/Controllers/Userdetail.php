...
class Userdetail extends Controller
{
    ...
    public function bidHistory(){
        ...
        $bidHistory = Userbit::where('userid', $user->id)->get();
        return view('bidhistory', compact('bidHistory'));
    }
    public function verifytransaction(Request $request){
        $userid = Session::get('userlogin');
        // dd($userid);
        $adminSettings = AdminSetting::latest()->first();
        if($adminSettings->wallet_bonus > 0)
            $totalAmount = $request->amount + ($request->amount * $adminSettings->wallet_bonus / 100);
        else
            $totalAmount = $request->amount;
        // dd($totalAmount);

        addtransaction($userid->id, 'upi', date("ydmhsi"), 'credit', $totalAmount, 'recharge', 'Pending', '0');
        return response()->json(["success" => true, "redirect" => route('deposit_withdrawals')]);
    }
}
