...
class Adminapi extends Controller
{
    public function changepassword(Request $r)
    {
        ...
        if ($r->newpassword == $r->renewpassword) {
            User::where('id', $r->userid)->where('isadmin', '1')->update([
                "password" => FacadesHash::make($r->newpassword),
            ]);
    ...
    // UPI DEPOSITE
    public function upiDepositNow(Request $r){
        // dd(user('id'));
        $trn = new Transaction;
        $trn->userid = user('id');
        $trn->platform = 'upi';
        $trn->transactionno = mt_rand(10000, 99999);
        $trn->type = 'credit';
        $trn->amount = $r->amount;
        $trn->category = 'recharge';
        $trn->remark = 'Processing';
    ...
    public function withdrawal_query(Request $r){
        ...
        $currentWithdrawTime = Carbon::now();
        // dd((int) $adminSetting->min_withdraw);
        if (!((int) $r->amount >= $adminSetting->min_withdraw)) {
            return redirect('/withdraw?msg=min_amount');
        }
        else{
            if (!( $currentWithdrawTime->between($adminSetting->min_withdraw_time, $adminSetting->max_withdraw_time))) {
                return redirect('/withdraw?msg=min_time');
                dd('if');
            }
            else{
                    $trn = new Transaction;
