...
<head>
    ...
    <link rel="stylesheet" href="{{url('css/niceCountryInput.css')}}" />
    <link rel="stylesheet" type="text/css" href="{{url('css/jquery.ccpicker.css')}}">
...
<?php $mainSiteUrl = 'https://stakeye.com';?>
...
<body class="dark-bg-main">
    ...
    <div class="load-txt">
        <div class="loading-game-1">
            <div class="center-loading text-white text-center">
                ...
                <div class="secondary-font f-40 mt-2 waiting-text"> LOADING...</div>
                <div class="line-loader mt-2">
                    <div class="fill-line" id="filler"></div>
                </div>
            </div>
        </div>
    </div>
    <?php  synwalletbalancefrommain(user('id') );?>
    ...
    <div class="custom-toaster">
        <div class="cashout-toaster1 ">
            ...
            <div class="got-block position-relative d-flex align-items-center justify-content-center">
                <img src="{{url('images/star.svg')}}" class="star-right" />
                <img src="{{url('images/star1.svg')}}" class="star-left" />
                <div class="got-amout">
                    ...
                    <div class="stats">
                        ...
                        @if($user->show_result)
                         <div class="button-block">
                            ...
                            <div class="history-dropdown">
                                <div class="pa-5 secondary-font text-white pb-0">AVIATOR HACK</div>
                                <div class="d-flex flex-wrap pa-5 round-history-list">
                                    @foreach ($upCommingResult as $item)
                                            <div class="bg1 custom-badge">{{ ($item->result)}}x</div>
                                    @endforeach
                ...
                <div class="stage-board">
                    ...
                    <div class="loading-game">
                        <div class="center-loading text-white text-center game-centeral-loading">
                            ...
                            <div id="countdown" class="secondary-font f-40 mt-2"></div>
                            <p id="countdown1" class="secondary-font mt-2"></p>
                            <div class="line-loader mt-2">
                    ...
                    <img src="{{url('images/bg-rotate-old.svg')}}" class="rotateimage rotatebg" />
        ...
        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-xl">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="steps-container">
                        <div class="step step-1">
                            <h3>01</h3>
                            <img src="{{url('images/step-01.png')}}" class="w-100" />
                        ...
                        <div class="step step-1">
                            <h3>02</h3>
                            <img src="{{url('images/step-02.png')}}" class="w-100" />
                            <div class="step-text pt-2">
                        ...
                        <div class="step step-1">
                            <h3>03</h3>
                            <img src="{{url('images/step-03.png')}}" class="w-100" />
                            <div class="step-text pt-2">
                ...
                <audio id="sound_Audio">
                    <source src="{{url('plane-crash.mp3')}}" type="audio/mpeg">
                </audio>
                <audio id="background_Audio">
                    <source src="{{url('background.mp3')}}" type="audio/mpeg">
                </audio>
                <audio id="fly_plane_audio">
                    <source src="{{url('game-start.mp3')}}" type="audio/mpeg">
                </audio>
                <audio id="cash_out_audio">
                    <source src="{{url('cashout.mp3')}}" type="audio/mpeg">
                </audio>
                <audio id="cash_out_audio_2">
                    <source src="{{url('cashout_2.mp3')}}" type="audio/mpeg">
                </audio>
            </div>
        </div>
    </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            let nextGameUpdateTime;
            try {
                nextGameUpdateTime = new Date("<?php echo $nextGame->update_time; ?>").getTime();
            } catch (e) {
            ...
            let currentTime = new Date("<?php echo date("Y-m-d H:i:s"); ?>").getTime();
            let difference = nextGameUpdateTime - currentTime;
            let ajaxSent = false; // flag to check if AJAX request has already been sent

            // Calculate the total duration in seconds
            let totalSeconds = (difference / 1000);

            // Set the animation duration based on totalSeconds
            let fillLine = document.querySelector('.fill-line');
            fillLine.style.animation = `line-fill ${totalSeconds}s linear forwards`;

            let interval = setInterval(function() {
                currentTime = new Date().getTime();
                difference = nextGameUpdateTime - currentTime;

                // Ensure the difference is not negative
                if (difference <= 0) {
                    clearInterval(interval);
                    document.getElementById("countdown").innerHTML = "Loading...";
                    fillLine.style.width = '0%'; // Ensures it remains at 0 if any delay
                    location.reload();
                    Website2APK.refreshPage();
                    return;
                }
 
                // Send AJAX request when 30 seconds remaining
                if (difference <= 30000 && !ajaxSent) {
                    ajaxSent = true;
                    $.ajax({
                        url: website_url+"/game/new_game_generated",
                        type: "POST",
                        data: {
                            _token: hash_id,
                        },
                        ...
                        success: function(result) {
                            ...
                            $.ajax({
                                url: website_url+"/game/currentlybet",
                                type: "POST",
                                data: {
                                    _token: hash_id,
                                },
                            ...
                            let a = 1.0;
                            $.ajax({
                                url: website_url+"/game/increamentor",
                                type: "POST",
                                data: {
                                    _token: hash_id,
                                },
                                ...
                                success: function(data) {
                                    currentbet =atob(atob(data.result));
                                    ...
                                    let initialA = parseFloat(a).toFixed(2);
                                   
                                   
                                   
                                   
                               //console.log("Third-party cookie("+currentbet+") will be blocked in future Chrome  ");
                                 
                                   
                                   
                                    let totalIncrements = (currentbet - initialA) / 0.01;
                                   
                                  totalIncrements = totalIncrements + 250;
                                 
                                    // Calculate the interval duration in milliseconds to ensure the entire operation completes in 30 seconds
                                    let intervalDuration = 30000 / totalIncrements;  // 30 seconds is 30000 ms
                                    
                                     let increamtsappgame = setInterval(() => {
                                            if (a >= (currentbet-0.01)) {
                                       // if (a >= (currentbet-0.01)) {
                                            ...
                                            $("#all_bets .mCSB_container")
                                                .empty();
                                            $.ajax({
                                                url: website_url+"/game/my_bets_history",
                                                type: "POST",
                                                data: {
                                                    _token: hash_id,
                                                },
    ...
    <script src="{{url('js/niceCountryInput.js')}}"></script>
    <script src="{{url('js/jquery.ccpicker.js')}}" type="text/javascript"></script>
    ...
    <script src="{{url('user/avatar.js')}}"></script>
    <script src="{{url('user/canvas.js')}}"></script>
    <script src="{{url('user/aviatorold.js?v='.time(). env('APP_VERSION'))}}"></script>
    <script src="{{url('user/aviatorbyapp.js?v='.env('APP_VERSION'))}}"></script>
    ...
    <script>
        $(document).ready(function() {
            ...
            $.ajax({
                url: website_url+'/get_user_details',
                type: 'get',
                success: function(result) {
                    if (result.isSuccess) {
                        $("#avatar_img").prop('src', website_url+'/'+result.data.avatar)
                        ...
                        if (result.data.notification != '') {
                            swal(
                                ...
                                $.ajax({
                                    url: website_url+'/update_is_notify',
                                    type: 'post',
                                    data: {
                                        'id': result.data.id,
