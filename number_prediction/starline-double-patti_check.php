...
if($wallet >= $total){
if(empty($number)){
	header("location: starline-double-patti.php?$date=$date&msg=Bet on atleast one number");
}else{
	$check_id="";
    ...
    if (empty($value)){
		...
		header("location: starline-double-patti.php?msg=Bet Accepted");
	}else{
		header("location: starline-double-patti.php?msg=You are late for this bet");
	}
}
}else{
	header("location: starline-double-patti.php?msg=Low Balance");
}
}else{
