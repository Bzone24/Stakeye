...
$stmt1 = $db->query("select NAME, ID from GAMES where PLAY='checked' and TIME1 < now() - INTERVAL 1 MINUTE and DAYS>= $day and (HOLIDAY='' or HOLIDAY is NULL);");
while ($row1 = $stmt1->fetch(PDO::FETCH_ASSOC)) {
    ...
    $result1 = "";
    $stmt2 = $db->query("select RESULT1 from RESULT where GAME_ID='$game_id' and DATE=CURDATE();");
    while ($row2 = $stmt2->fetch(PDO::FETCH_ASSOC)) {
    ...
    if ($result1 != "") {
        ...
        $stmt2 = $db->query("select ID, AMOUNT, USER_ID from BET_TRANSACTIONS where GAME_ID='$game_id' and NUMBER='$andar' and TYPE='Andar' and GAME='andar' and (STATUS='' or STATUS is NULL) and DATE=CURDATE();");
        while ($row2 = $stmt2->fetch(PDO::FETCH_ASSOC)) {
            $bet_id = $row2['ID']??'';
            $amount = $row2['AMOUNT']??'';
        ...
        //Bahar Result Check
        $stmt2 = $db->query("select ID, AMOUNT, USER_ID from BET_TRANSACTIONS where GAME_ID='$game_id' and NUMBER='$bahar' and TYPE='Bahar' and GAME='bahar' and (STATUS='' or STATUS is NULL) and DATE=CURDATE();");
        while ($row2 = $stmt2->fetch(PDO::FETCH_ASSOC)) {
            $bet_id = $row2['ID']??'';
            $amount = $row2['AMOUNT']??'';
            $user_id = $row2['USER_ID']??'';
        ...
        //Jodi Result Check
        $stmt2 = $db->query("select ID, AMOUNT, USER_ID from BET_TRANSACTIONS where GAME_ID='$game_id' and NUMBER='$result1' and TYPE='Jodi' and GAME='JODI' and (STATUS='' or STATUS is NULL) and DATE=CURDATE();");
        while ($row2 = $stmt2->fetch(PDO::FETCH_ASSOC)) {
            $bet_id = $row2['ID']??'';
            $amount = $row2['AMOUNT']??'';
            $user_id = $row2['USER_ID']??'';
        ...
        $stmt2 = $db->query("select ID, AMOUNT,NUMBER, USER_ID from BET_TRANSACTIONS where GAME_ID='$game_id'    and FIND_IN_SET('$result1', NUMBER)  and TYPE='Cross' and GAME='CROSS' and (STATUS='' or STATUS is NULL) and DATE=CURDATE();");
        while ($row2 = $stmt2->fetch(PDO::FETCH_ASSOC)) {
            $bet_id = $row2['ID']??'';
            $amount = $row2['AMOUNT']??'';
            $user_id = $row2['USER_ID']??'';
            $totalNumbers = explode(",", str_replace(" ","",$row2['NUMBER']));
            
            
            $win_amount = $amount * ($jodi/count($totalNumbers));
            $stmt3 = "update BET_TRANSACTIONS set RESULT='PASS', WIN_AMOUNT='$win_amount', STATUS='CHECKED' where ID='$bet_id';";
            $db->query($stmt3);
            $stmt3 = "update USERS set WALLET=WALLET + $win_amount where ID='$user_id';";
            $db->query($stmt3);
          ...
          $stmt3 = "update BET_TRANSACTIONS set RESULT='FAIL', STATUS='CHECKED' where GAME_ID='$game_id' and TYPE='Cross' and GAME='CROSS' and (STATUS='' or STATUS is NULL) and DATE=CURDATE();";
        $db->query($stmt3);
