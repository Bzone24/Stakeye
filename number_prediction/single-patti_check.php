...
if($wallet >= $total){
if(empty($number)){
	header("location: single-patti.php?$date=$date&msg=Bet on atleast one number");
}else{
	$check_id="";
	if($open_close=="open"){
		$stmt2 = $db->query("select ID from GAMES where ID='$id' and TIME1 > now() + INTERVAL 10 MINUTE  and DAYS >= $day;");
	}else{
		if($id=="14000000000000000000"){
    ...
    if (empty($value)){
		...
		header("location: single-patti.php?msg=Bet Accepted");
	}else{
		header("location: single-patti.php?msg=You are late for this bet");
	}
}
}else{
	header("location: single-patti.php?msg=Low Balance");
}
}else{
