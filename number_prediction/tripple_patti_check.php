...
if($wallet >= $total){
if(empty($number)){
	header("location: tripple-patti.php?$date=$date&msg=Bet on atleast one number");
}else{
	$check_id="";
	if($open_close=="open"){
    ...
    if (empty($value)){
		...
		echo "$num,$value <br/>";
        $stmt1 = "insert into BET_TRANSACTIONS (USER_ID,DATE_TIME,AMOUNT,GAME_ID,GAME,NUMBER,STATUS,RESULT,RAND,DATE,TYPE) values ('$user_id',now(),'$value','$id','$open_close','$num','','','$rand','$date','Tripple Patti');";
		$db->query($stmt1);
		$bet_id="";
		...
		header("location: tripple-patti.php?msg=Bet Accepted");
	}else{
		header("location: tripple-patti.php?msg=You are late for this bet");
	}
}
}else{
	header("location: tripple-patti.php?msg=Low Balance");
}
}else{
