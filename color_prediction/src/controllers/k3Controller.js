...
const rosesPlus = async (auth, money) => {
    ...
    const [user] = await connection.query('SELECT `id`, `phone`, `code`, `invite` FROM users WHERE token = ? AND veri = 1  LIMIT 1 ', [auth]);
    let userInfo = user[0];
    ...
    if (money >= 10000) {
        if (f1.length > 0) {
            ...
            try {
                await balanceHelper.updateBalanceInMainWallet(infoF1.phone, rosesF1, '+','phone');
             } catch (error) {
            ...
            if (f2.length > 0) {
                ...
                try {
                    await balanceHelper.updateBalanceInMainWallet(infoF2.phone, rosesF2, '+','phone');
                 } catch (error) {
                ...
                if (f3.length > 0) {
                    ...
                    try {
                        await balanceHelper.updateBalanceInMainWallet(infoF3.phone, rosesF3, '+','phone');
                     } catch (error) {
                    ...
                    if (f4.length > 0) {
                        ...
                        try {
                            await balanceHelper.updateBalanceInMainWallet(infoF4.phone, rosesF4, '+','phone');
                         } catch (error) {
...
const betK3 = async (req, res) => {
    try {
        ...
        if (check >= 0) {
            ...
            try {
                await balanceHelper.updateBalanceInMainWallet(userInfo.id, total, '-');
             } catch (error) {
