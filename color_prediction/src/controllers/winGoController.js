...
const rosesPlus = async (auth, money) => {
    ...
    const [user] = await connection.query('SELECT `id`,`phone`, `code`, `invite`, `user_level`, `total_money` FROM users WHERE token = ? AND veri = 1 LIMIT 1 ', [auth]);
    let userInfo = user[0];
    const [f1] = await connection.query('SELECT `id`,`phone`, `code`, `invite`, `rank`, `user_level`, `total_money` FROM users WHERE code = ? AND veri = 1 LIMIT 1 ', [userInfo.invite]);
            ...
            for (let levelIndex = 1; levelIndex <= 6; levelIndex++) {
                ...
                if (infoF1.user_level >= levelIndex && infoF1.total_money >= 100) {
                    ...
                    if (rosesF > 0) {
                        ...
                        try {
                            await balanceHelper.updateBalanceInMainWallet(infoF1.phone, rosesF, '+','phone');
                         } catch (error) {
...
const betWinGo = async (req, res) => {
    ...
    if (check >= 0) {
        ...
        today = ?,
        time = ?`;
        await connection.execute(sql, [id_product, userInfo.phone, userInfo.code, userInfo.invite, period, userInfo.level, total, x, fee, gameJoin, join, 0, checkTime, timeNow]);
        await connection.execute('UPDATE `users` SET `money` = `money` - ? WHERE `token` = ? ', [money * x, auth]);
 

        try {
            await balanceHelper.updateBalanceInMainWallet(auth, money * x, '-','token');
         } catch (error) {
...
const handlingWinGo1P = async (typeid) => {
    ...
    for (let i = 0; i < order.length; i++) {
        ...
        const [users] = await connection.execute('SELECT `id`,`money` FROM `users` WHERE `phone` = ?', [phone]);
        ...
        try {
            await balanceHelper.updateBalanceInMainWallet(users[0].id, parseFloat(nhan_duoc), '+');
         } catch (error) {
