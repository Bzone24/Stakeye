...
const addUserAccountBalance = async ({ money, phone, invite }) => {
   ...
   try {
      await balanceHelper.updateBalanceInMainWallet(phone, user_money, '+','phone');
  } catch (error) {
   ...
   if (inviter.length) {
      ...
      try {
         await balanceHelper.updateBalanceInMainWallet(inviter?.[0].phone, inviter_money, '+','phone');
     } catch (error) {
...
const settingbuff = async (req, res) => {
   ...
   if (user_id.length > 0) {
      if (buff_acc == "1") {
         ...
         try {
            await balanceHelper.updateBalanceInMainWallet(id_user, money_value, '+','id_user');
        } catch (error) {
